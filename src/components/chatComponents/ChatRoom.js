...
export default function ChatRoom(){
    ...
    function handleSubmit(e){
        ...
        if(inputRef.current.value.length !== 0) {
            databaseRef.child(`messages/${roomId}/${id}`).set(message).then(r => {
    ...
    async function loadChat(){
        await databaseRef.child(`messages/${roomId}`).once('value', snap => {
                    ...
                    loadUsersFromMessages(messageList).then(profileList => {
                        for (let profile of profileList) {
                            messageList = messageList.map(message => {
                                if (message.authorId === profile.uid) message.profile = profile
                                return message
                            })
                        }
            ...
            console.log("Call to render messages took " + (t1 - t0) + " milliseconds.")
        })
    }
     function updateMessages(){
        ...
        let skipQuery = true
        databaseRef.child(`messages/${roomId}`).endAt().limitToLast(1).on('child_added',  (snap) => {
            var t0 = performance.now()
            if (!loading && !skipQuery) {
                ...
                if (!userList.some(user => user.uid === authorId)){
                   Promise.resolve(getUserFromId(authorId)).then(newUser => userList.push(newUser))
                }
                msg.profile = userList.filter(user => user.uid === authorId)[0]
                if (msg.profile){
            ...
            skipQuery = false
            var t1 = performance.now()
            console.log("Call to update messages took " + (t1 - t0) + " milliseconds.")
        })
    }
    ...
    useEffect(()=> {
        ...
        Promise.all([getRoomInfo(), loadChat()]).then(r => {
            setLoading(false)
        ...
        return () => {
            ...
            databaseRef.child(`messages/${roomId}`).endAt().limitToLast(1).off('child_added')
    ...
    return (
        <div className={"chatbox-container"}>
            ...
            <div className="column-container chatbox" >
                ...
                <div className="chatbox-main" style={{visibility: scrolled ? "visible" : "hidden"}} ref={chatboxRef} onLoad={() => scrollBottomAtStart()} onScroll={() => handleScroll()} onClick={() => {if (profileToShow) setProfileToShow(null)}}>
                    {messages && !messages.some(message => message.profile === undefined)  &&  messages.map((message, index) => {
    ...
    d="M476 3.2L12.5 270.6c-18.1 10.4-15.8 35.6 2.2 43.2L121 358.4l287.3-253.2c5.5-4.9 13.3 2.6 8.6 8.3L176 407v80.5c0 23.6 28.5 32.9 42.5 15.8L282 426l124.6 52.2c14.2 6 30.4-2.9 33-18.2l72-432C515 7.8 493.3-6.8 476 3.2z"/>
            ...
            {isDesktop &&  <ViewPeople loading={loading} setProfileToShow={setProfileToShow} users={users}/>}
        </div>
